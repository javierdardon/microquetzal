...
// Build received from controller
...
// Form that will be sent to controller to update
const form = reactive({
  id: props.build.id,
  sale: applyOffer,
...
// Watchers for price and percentange changes
watch(percentage, (newPercentage) => {
  if (applyOffer.value) {
    calculatedPrice.value = (price.value - (price.value * ( newPercentage / 100 ))).toFixed(2);
  } else {
...
watch(price, (newPrice) => {
  if (applyOffer.value) {
    calculatedPrice.value = (newPrice - (newPrice * ( percentage.value / 100 ))).toFixed(2);
  } else {
    calculatedPrice.value = 0;
  }
});
...
// Form submission
function submit() {
  router.post('/admin/builds-update-sale', form)
}
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-5 w-full">
          ...
          <p class="dark:text-white text-xl my-7"> {{ props.build.name }} </p>
          <div :style="image" class="rounded-lg h-48 sm:h-96 w-48 sm:w-96 bg-no-repeat bg-center bg-cover mb-4"></div>
          ...
          >
              ...
              <div v-if="applyOffer" class="my-8">
                  ...
                  <div class="mb-6">
                      <p class="dark:text-white text-sm sm:text-lg mb-2">
                          Precio original:
                          <span class="ml-2 text-sm lg:text-lg"> Q {{ price || 0 }} </span>
                      </p>
                      <p class="dark:text-white text-sm lg:text-xl mb-2">
                          Precio ofertado:
